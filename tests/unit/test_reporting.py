...
def test_is_transient_github_error(response_code, is_transient):
    """Test classification of GitHub API errors."""
    exception = flexmock(response_code=response_code)

    assert StatusReporterGithubStatuses._is_transient_github_error(exception) == is_transient
...
def test_github_checks_error_handling(reraise_transient_errors, response_code):
    ...
    is_transient = StatusReporterGithubStatuses._is_transient_github_error(exception)
...
def test_github_statuses_error_handling(reraise_transient_errors, response_code):
    """Test error handling in StatusReporterGithubStatuses."""
    project = flexmock()
    reporter = flexmock(
        StatusReporterGithubStatuses(
            project=project,

...
from packit_service.worker.helpers.open_scan_hub import CoprOpenScanHubHelper, KojiOpenScanHubHelper
...
class KojiOpenScanHubAbstractHandler(OpenScanHubAbstractHandler):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def get_helper(self) -> KojiOpenScanHubHelper:
        build_helper = KojiBuildJobHelper(
            service_config=self.service_config,
            package_config=self.package_config,
            project=self.project,
            metadata=self.data,
    ...
    def check_scan_and_build(self):
        ...
        if not self.event.scan or not self.event.build:
            return TaskResults(
                success=True,
                details={
                    "msg": f"Scan {task_id} not found or not associated with a Koji build",
                },
            )
...
class KojiOpenScanHubTaskFinishedHandler(
    KojiOpenScanHubAbstractHandler,
):
    event: openscanhub.task.Finished
    task_name = TaskName.openscanhub_task_finished
...
@configured_as(job_type=JobType.koji_build)
@reacts_to(openscanhub.task.Started)
class KojiOpenScanHubTaskStartedHandler(
    KojiOpenScanHubAbstractHandler,
):
    task_name = TaskName.openscanhub_task_started

...
@celery_app.task(bind=True, name=TaskName.downstream_testing_farm_tests_ns, base=TaskWithRetry)
def run_downstream_testing_farm_tests_ns_handler(
    self,
    event: dict,
    package_config: dict,
    job_config: dict,
    ...
    handler = DownstreamTestingFarmTestsNSHandler(
        package_config=load_package_config(package_config),
        job_config=load_job_config(job_config),
        event=event,
        celery_task=self,
